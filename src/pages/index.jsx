...
import { GirlImage, MarkdownGirl } from "../components/girls/girls";
...
export default ({ data: { girls, tweets, users } }) => {
  ...
  const allUsers = users.edges.reduce(
  ...
  const tweetInfo = allTweets.map(tweet => ({
    ...tweet,
    ...allUsers[tweet.name]
  }));
    ...
    <Layout>
      <PerfectScrollbar>
        ...
        <GirlsHeader
          ...
          className="girls-header-box"
          roundedHeader
        >
          <Subtitle className="is-size-7 has-text-white-bis">
            Click portraits to view details...
          </Subtitle>
        </GirlsHeader>
        <div className="girls">
